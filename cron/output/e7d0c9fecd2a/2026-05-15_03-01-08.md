...
**Hermes GitHub Backup — 2026-05-15 03:00**

**Status: FAILED** — No GitHub token available.

**Diagnostic:**
- `GIT_TOKEN` / `GITHUB_TOKEN` env var: ❌ not set
- `~/.git-credentials`: ❌ not found
- `~/.config/gh/credentials`: ❌ not found
- `/tmp/git-cred-store`: ❌ empty

**Action Required:**
Export a GitHub PAT before the next scheduled run:
```bash
export GIT_TOKEN=ghp_your_36_char_token_here
```
Or configure `git credential store` with a valid Classic PAT (`ghp_...`) or Fine-grained token (`REDACTED`).

Without a token the shallow-clone overlay backup cannot authenticate to `zhengyu-git/hermes-agent`. The next cron run will fail the same way until a token is provided.
