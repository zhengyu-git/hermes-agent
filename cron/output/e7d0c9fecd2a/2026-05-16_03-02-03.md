...
| 克隆方式 | ✅ Shallow clone overlay |
| 同步目录 | skills, memories, cron, perfsight_docs, perfsight_openapi_docs |
| 脱敏文件 | 40 个文件 (config_sanitized.yaml, cron/jobs.json, cron/output/*.md 等) |
| 提交 | ✅ `hermes full backup 2026-05-16 03:01` |
| 推送 | ✅ `master -> master` (a4f3586..1744c90) |

**推送内容摘要**: 119 files changed, 2595 insertions(+), 432 deletions(-)

注意: `~/.git-credentials` 中有 `REDACTED` 占位符残留（被上次备份的脱敏步骤覆盖），已改用 `~/.netrc` 认证方式避免此问题。
