...
### Token Validity Debugging (Pre-flight Check)

Before any git operation, verify the token is actually valid:

```bash
TOKEN=ghp_your_token_here
# Returns authenticated user info if token is valid
curl -s -u "x-access-token:$TOKEN" https://api.github.com/user
```

**Response patterns:**
| Response | Meaning |
|----------|---------|
| `401 Bad Credentials` | Token is revoked/expired/invalid — replace it |
| `Requires authentication` | Auth passed but needs username — try `x-access-token` prefix |
| `{"login": "ZhengYu", ...}` | Token valid, proceed with git clone |
| Repo returns 404 without auth | Repository is private, token has no access — grant repo permission |
...
Backup failed: No GitHub token found (GIT_TOKEN/GITHUB_TOKEN not set).

This cron job requires a GitHub Personal Access Token to be set in the environment. Please ensure `GIT_TOKEN` or `GITHUB_TOKEN` is exported before the cron job runs, e.g.:

```bash
export GIT_TOKEN=ghp_your_token_here
```

Or configure via credential store: `git config --global credential.helper store`

Once a token is available, the backup will proceed normally.
