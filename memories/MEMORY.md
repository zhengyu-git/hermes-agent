...
§
PerfInsight查询（默认）：先conditionalSearch（platform=5/source=1）查最新对局，再通过detail接口（GET参数：username/Appid/token/Idstr）查帧率。凭证：App Key=116a83402fa27c74fe0e571f4a32a32a69d, App ID=348901537, username=hfvjj, env=v2。JWT：exp=time+120s，alg=HS256，每次查重新生成token。详见~/.hermes/perfsight_openapi_docs/
§
placeholder
§
...
§
用户希望将hermes核心文件（skills/、memories/、cron配置、PerfInsight文档等）备份到Gitee做灾备。仓库待创建。
§
...
【Gitee备份】
- 仓库：https://gitee.com/zhengyu-git/mermes-agent
- 令牌：3ddc84dbbd5a6582eed8b02e0a809373
- 本地打包文件：/tmp/mermes-backup.tar.gz（2.4MB，503个文件）
- 状态：Gitee网络不通，待用户在公司网络环境手动push
